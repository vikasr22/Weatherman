...
function getScene(weather: WeatherData | null): Scene {
  ...
  // Temperature-based snow background: if below 2°C, show snow
...
/* ─── Per-scene text palettes ────────────────────────────── */
// Each scene gets its own tuned t1/t2/t3 so text is always legible
// against that specific background. t1 = primary, t2 = secondary, t3 = muted.
const SCENE_TEXT: Record<Scene, { t1: string; t2: string; t3: string; light: boolean }> = {
  // Dark sky scenes → light text (improved contrast)
  "night":      { t1: "#ffffff", t2: "#c8d5e8", t3: "#8a9aB0", light: false },
  "rain":       { t1: "#f5f7ff", t2: "#b8c8e0", t3: "#8a9ab8", light: false },
  "heavy-rain": { t1: "#f0f3ff", t2: "#b0c0e0", t3: "#7a8aa8", light: false },
  "thunder":    { t1: "#ffffff", t2: "#c0d0f0", t3: "#8a9aba", light: false },
  "default":    { t1: "#ffffff", t2: "#c8d5e8", t3: "#8a9ab0", light: false },
  "day-clear":  { t1: "#ffffff", t2: "#d0e8ff", t3: "#a0c8e8", light: false },
  "day-cloudy": { t1: "#ffffff", t2: "#b8c8e0", t3: "#7a8a9a", light: false },

  // Light/bright sky scenes → dark text
  "day-hot":  { t1: "#1a1200", t2: "#4a3800", t3: "#7a6020", light: true },
  "sunrise":  { t1: "#1a0f00", t2: "#4a2800", t3: "#7a4800", light: true },
  "sunset":   { t1: "#120008", t2: "#380020", t3: "#680040", light: true },
  "fog":      { t1: "#18202a", t2: "#384858", t3: "#506070", light: true },

  // Snow — bright white sky → dark slate text
  "snow":     { t1: "#0d1a2a", t2: "#2a3a50", t3: "#4a5a70", light: true },
};

function getSceneTextColors(scene: Scene) {
  return SCENE_TEXT[scene] ?? SCENE_TEXT["default"];
}
...
function WeatherCanvas({ scene }: { scene: Scene }) {
  ...
  useEffect(() => {
    ...
    const drawCloud = (
      ...
      puffs: { dx: number; dy: number; rs: number }[]
    ) => {
    ...
    const draw = () => {
      ...
      if (sc === "night") {
        ...
        aur.addColorStop(0,"rgba(30,80,60,0.07)"); aur.addColorStop(1,"transparent");
        ...
        sunG.addColorStop(0,"rgba(255,230,80,0.9)"); sunG.addColorStop(0.3,"rgba(255,160,30,0.5)"); sunG.addColorStop(1,"transparent");
        ...
        clouds.forEach(c => { c.x=(c.x+c.speed)%1.15; drawCloud(c.x*w,c.y*h,c.w,c.h,c.alpha*0.7,"#ffb86c",c.puffs); });
        ...
        sunG.addColorStop(0,"rgba(255,210,0,0.85)"); sunG.addColorStop(0.25,"rgba(249,120,30,0.5)"); sunG.addColorStop(1,"transparent");
        ctx.fillStyle=sunG; ctx.fillRect(0,0,w,h);
        ctx.beginPath(); ctx.arc(sx,sy,Math.min(w,h)*0.05,0,Math.PI*2);
        ctx.fillStyle="#ffe066"; ctx.fill();
        clouds.forEach(c => { c.x=(c.x+c.speed*0.6)%1.15; drawCloud(c.x*w,c.y*h,c.w,c.h,c.alpha*0.65,"#c87941",c.puffs); });
      }
        ...
        sunG.addColorStop(0,"rgba(255,248,180,0.45)"); sunG.addColorStop(1,"transparent");
        ctx.fillStyle=sunG; ctx.fillRect(0,0,w,h);
        ctx.beginPath(); ctx.arc(sx,sy,Math.min(w,h)*0.042,0,Math.PI*2);
        ctx.fillStyle="#fffbe0"; ctx.fill();
        clouds.slice(0,4).forEach(c => { c.x=(c.x+c.speed*0.7)%1.15; drawCloud(c.x*w,c.y*h,c.w,c.h,c.alpha*0.6,"#ffffff",c.puffs); });
      }
        ...
        const sunG = ctx.createRadialGradient(sx,sy,0,sx,sy,w*0.3);
        sunG.addColorStop(0,"rgba(255,220,50,0.7)"); sunG.addColorStop(0.4,"rgba(255,140,0,0.3)"); sunG.addColorStop(1,"transparent");
        ctx.fillStyle=sunG; ctx.fillRect(0,0,w,h);
        ...
        heatWaves.forEach(hv => {
          ...
          wg.addColorStop(0,`rgba(255,200,50,${hv.alpha})`); wg.addColorStop(1,"transparent");
        ...
        ctx.fillRect(0,0,w,h);
        clouds.forEach(c => { c.x=(c.x+c.speed*0.5)%1.15; drawCloud(c.x*w,c.y*h,c.w*1.2,c.h*1.2,c.alpha*0.75,"#dce8f0",c.puffs); });
      }

      else if (sc === "rain") {
        ctx.fillStyle = skyGrad(["#1a2030","#253050","#2e3a5a","#384260"]);
        ctx.fillRect(0,0,w,h);
        clouds.forEach(c => { c.x=(c.x+c.speed*0.3)%1.15; drawCloud(c.x*w,c.y*h*0.6,c.w*1.4,c.h*1.5,c.alpha*0.8,"#2a3040",c.puffs); });
        ctx.save();
        drops.forEach(d => {
          d.y+=d.speed; if(d.y>1){d.y=-0.05;d.x=Math.random();}
          ctx.beginPath(); ctx.moveTo(d.x*w,d.y*h); ctx.lineTo(d.x*w-1.5,(d.y+d.len)*h);
          ctx.strokeStyle=`rgba(160,200,255,${d.alpha})`; ctx.lineWidth=0.8; ctx.stroke();
        });
        ctx.restore();
      }
        ...
        ctx.fillRect(0,0,w,h);
        clouds.forEach(c => { c.x=(c.x+c.speed*0.2)%1.15; drawCloud(c.x*w,c.y*h*0.5,c.w*1.6,c.h*1.8,0.9,"#161e2a",c.puffs); });
        ctx.save();
        heavyDrops.forEach(d => {
          d.y+=d.speed; if(d.y>1.05){d.y=-0.08;d.x=Math.random();}
          ctx.beginPath(); ctx.moveTo(d.x*w,d.y*h); ctx.lineTo(d.x*w-3,(d.y+d.len)*h);
          ctx.strokeStyle=`rgba(140,190,255,${d.alpha})`; ctx.lineWidth=1.2; ctx.stroke();
        });
        ctx.restore();
        for(let i=0;i<6;i++){
          const rx=((i*0.17+ts*0.05)%1)*w, ry=h*0.96, rs=((ts*0.8+i)%1)*20;
          ctx.beginPath(); ctx.ellipse(rx,ry,rs*1.5,rs*0.3,0,0,Math.PI*2);
          ctx.strokeStyle=`rgba(160,210,255,${0.3*(1-rs/20)})`; ctx.lineWidth=0.8; ctx.stroke();
        }
      }

      else if (sc === "thunder") {
        nextLightning -= 0.016;
        if(nextLightning<=0){ lightningFlash=0.4+Math.random()*0.4; nextLightning=1.5+Math.random()*5; }
        ...
        if(flashAlpha>0.1){
          ...
          fl.addColorStop(0,"rgba(200,220,255,0.6)"); fl.addColorStop(1,"transparent");
          ...
          let by=0;
          while(by<h*0.65){ by+=18+Math.random()*30; ctx.lineTo(bx+(Math.random()-0.5)*60,by); }
          ctx.stroke(); ctx.restore();
        }
        clouds.forEach(c => { c.x=(c.x+c.speed*0.15)%1.15; drawCloud(c.x*w,c.y*h*0.45,c.w*1.8,c.h*2,0.92,"#0a0e18",c.puffs); });
        ctx.save();
        heavyDrops.forEach(d => {
          d.y+=d.speed*1.3; if(d.y>1.05){d.y=-0.08;d.x=Math.random();}
          ctx.beginPath(); ctx.moveTo(d.x*w,d.y*h); ctx.lineTo(d.x*w-4,(d.y+d.len*1.2)*h);
          ctx.strokeStyle=`rgba(120,170,255,${d.alpha*0.9})`; ctx.lineWidth=1.4; ctx.stroke();
        });
        ctx.restore();
        ...
        // Bright icy sky — white at top fading to pale blue-grey at bottom
        ctx.fillStyle=skyGrad(["#dce8f4","#e8f2fa","#f0f6fc","#f8fbff","#ffffff"]);
        ctx.fillRect(0,0,w,h);
        // Subtle horizon glow
        const hg=ctx.createLinearGradient(0,h*0.55,0,h);
        hg.addColorStop(0,"rgba(200,220,240,0.35)"); hg.addColorStop(1,"rgba(240,248,255,0.0)");
        ctx.fillStyle=hg; ctx.fillRect(0,0,w,h);
        // Heavy white/silver clouds
        clouds.forEach(c => { c.x=(c.x+c.speed*0.2)%1.15; drawCloud(c.x*w,c.y*h*0.5,c.w*1.5,c.h*1.6,0.72,"#d0dcea",c.puffs); });
        // Snowflakes — dark enough to show on white
        flakes.forEach(f => {
          f.y+=f.speed; f.x+=f.drift+Math.sin(ts*0.5+f.phase)*0.0003;
          ...
          ctx.fillStyle=`rgba(140,170,200,${f.alpha*pulse})`; ctx.fill();
        });
        // Snowy ground strip at bottom
        const sg=ctx.createLinearGradient(0,h*0.88,0,h);
        sg.addColorStop(0,"rgba(220,235,248,0.0)"); sg.addColorStop(1,"rgba(235,245,255,0.85)");
        ...
        for(let i=0;i<5;i++){
          ...
          fg.addColorStop(0,`rgba(200,210,215,${0.12+i*0.03})`); fg.addColorStop(1,"transparent");
  ...
  return (
    <div className="dial-wrap">
      <svg viewBox="0 0 120 120" className="dial-svg">
        ...
        <line x1={cx} y1={cy}
          x2={cx - (needleLen * 0.4) * Math.sin(needleRad)}
          y2={cy + (needleLen * 0.4) * Math.cos(needleRad)}
          stroke="rgba(255,255,255,0.18)" strokeWidth="1.5" />
        <line x1={cx} y1={cy}
          x2={cx + needleLen * Math.sin(needleRad)}
          y2={cy - needleLen * Math.cos(needleRad)}
          className="needle" />
...
export default function App() {
  ...
  const tc       = getSceneTextColors(scene);
  const isLight  = tc.light;
  ...
  return (
    <div className="app">
      ...
      <div
        className="shell"
        data-light-scene={isLight ? "true" : "false"}
        style={{
          "--text-1": tc.t1,
          "--text-2": tc.t2,
          "--text-3": tc.t3,
          ...
          {showSuggestions && (suggLoading || suggestions.length > 0) && (
            <ul className="suggestions-list" role="listbox">
              ...
              {suggestions.map((s, i) => (
                ...
                >
                  <SearchIcon />
                  <span className="suggestion-name">{s.name}{s.state ? `, ${s.state}` : ""}</span>
